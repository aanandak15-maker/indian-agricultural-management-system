...
const StatsPage = () => {
  ...
  const [performanceData, setPerformanceData] = useState<PerformanceData[]>([
    { name: 'Rice Yield', current: 75, target: 85, unit: 't/ha' },
    { name: 'Cotton Quality Export', current: 88, target: 95, unit: '%' },
    { name: 'Wheat Profitability', current: 70, target: 80, unit: '%' },
    { name: 'Organic Certification', current: 25, target: 40, unit: '%' },
    { name: 'Sugarcane Innovation', current: 60, target: 75, unit: '%' },
  ]);
  
  useEffect(() => {
    const initialSync = setTimeout(() => {
      console.log('Fields, Crops and Finance modules are now connected to statistics');
    }, 1000);
  ...
  const handleTableUpdate = (rowIndex: number, columnId: string, value: any) => {
    ...
    console.log(`Connected modules have been informed of the ${updatedRow.name} update`);
  };
  ...
  const handleViewChange = (view: 'performance' | 'harvest' | 'detailed') => {
    ...
    console.log(`Connected modules have been adapted to ${view === 'performance' ? 'indicators' : view === 'harvest' ? 'harvest' : 'detailed'} view`);
  };
